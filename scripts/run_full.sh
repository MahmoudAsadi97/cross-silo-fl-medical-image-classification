#!/usr/bin/env bash
# Full-tier launcher for the FINAL report numbers. Resilient: each run is independent
# and failures don't abort the batch (so an overnight run survives a single crash).
#
#   DATA_ROOT=$HOME/fl_data/fed_isic2019/raw bash scripts/run_full.sh cuda "0 1 2"
#
# Heavy: image 200, 50 rounds, all data. Expect MANY hours. Tips:
#   * run inside tmux/screen so it survives a disconnect;
#   * to trade a little quality for ~4x speed, prepend  IMG=128 ROUNDS=40  (see below).
set -uo pipefail   # NOTE: no -e, so one failed run does not kill the batch

DEVICE="${1:-cuda}"
SEEDS="${2:-0 1 2}"
IMG="${IMG:-}"; ROUNDS="${ROUNDS:-}"           # optional overrides (e.g. IMG=128 ROUNDS=40)
...
echo "=== FULL-TIER BATCH | device=$DEVICE seeds=[$SEEDS] img=${IMG:-200} $(date) ==="
...
# DP privacy-utility sweep at full tier (1 seed; DP needs big batch + BMM at img 200).
SEED=0
