...
from src.fl.fedprox import train_fedprox_client, weighted_average_state_dicts
...
def main():
    fl_cfg = load_yaml(PROJECT_ROOT / "configs/fl/fedprox.yaml")
    ...
    run_dir = PROJECT_ROOT / "results" / "fedprox_baseline"
    ...
    logger = get_logger("fedprox_baseline", str(log_dir / "train.log"))
    logger.info("Starting FedProx baseline")
    logger.info(f"Using device: {device}")
    logger.info(f"Using mu: {mu}")
    ...
    for round_idx in range(1, rounds + 1):
        ...
        for client_id in selected_clients:
            ...
            client_result = train_fedprox_client(
                client_id=client_id,
            ...
            logger.info(
                ...
                f"ce_loss={final_local_metrics['ce_loss']:.4f} | "
                f"prox_loss={final_local_metrics['prox_loss']:.4f} | "
    ...
    logger.info("FedProx training completed")
